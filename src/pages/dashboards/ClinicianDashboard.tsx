...
export default function ClinicianDashboard() {
  ...
  const quickActions = [
    ...
    { label: "My Profile", icon: UserPlus, route: "/health", color: "from-blue-500/20 to-blue-600/20" },
    ...
    >
          ...
          <div className="space-y-2">
            {recentScans.slice(0, 5).map((scan) => (
              <motion.div
                key={scan.id}
                whileHover={{ x: 2 }}
                className="bg-slate-800/40 rounded-xl p-4 border border-slate-700/40"
              >
      ...
      {pendingReviews > 0 && (
        <section className="space-y-3">
          ...
          <motion.button
            whileHover={{ x: 2 }}
            className="w-full flex items-center gap-3 p-3 bg-slate-800/40 rounded-xl border border-slate-700/40"
          >
      ...
      {/* AI Quick Tip */}
      <section className="bg-gradient-to-br from-purple-900/20 to-pink-900/20 rounded-2xl p-4 border border-purple-500/20">
