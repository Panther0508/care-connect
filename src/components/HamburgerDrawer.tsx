...
const ROLE_SHORTCUTS: Record<string, { label: string; icon: React.ElementType; route: string; roles: string[] }[]> = {
  ...
  clinician: [
    ...
    { label: "My Profile", icon: Users, route: "/health", roles: ["clinician"] },
  ],
  chw: [
...
export default function HamburgerDrawer({ open, onOpenChange }: HamburgerDrawerProps) {
  ...
  const getMenuItems = () => {
    ...
    return [...shortcuts, ...SECONDARY_ITEMS];
  };
        ...
        <div className="flex-1 overflow-y-auto py-6 px-4 space-y-2">
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <div className="p-2 rounded-xl bg-teal-500/10 text-teal-400 group-hover:text-teal-300 transition-colors">
                  <Icon size={20} />
                </div>
                <span className="flex-1 text-left text-white font-medium">{item.label}</span>
                <svg
                  className="w-4 h-4 text-slate-500 group-hover:text-teal-400"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
